...
// Setup Chart Parameters/Dimensions
  ...
  function xScale(peopleData, chosenXAxis) {
    ...
    var xLinearScale = d3.scaleLinear()
      .domain([d3.min(peopleData, d => d[chosenXAxis]) * 0.95,
        d3.max(peopleData, d => d[chosenXAxis])*1.05
      ])
      .range([0, width]);
  ...
  d3.csv("assets/data/data.csv").then(function(data, err) {
    ...
    data.forEach(function(incomeData) {
        ...
        incomeData.obesity = +incomeData.obesity/100;
    ...
    var yLinearScale = d3.scaleLinear()
        .domain([0.2, d3.max(data, d => d.obesity)+0.02])
        .range([height, 0]);
  ...
  // Append State Abbreviation Text to Circles
    var textGroup = chartGroup.selectAll(".stateAbbr")
        .data(data)
        .enter()
        .append("text")
        .attr("x", d => xLinearScale(d[chosenXAxis]))
        .attr("y", d => yLinearScale(d.obesity)*1.02)
        .text(d => (d.abbr))
    ...
    labelsGroup.selectAll("text")
        ...
        if (value !== chosenXAxis) {
            ...
            circlesGroup = updateToolTip(chosenXAxis, circlesGroup, textGroup, xLinearScale);
